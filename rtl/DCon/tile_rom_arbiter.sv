...
// tile_rom_arbiter — Round-robin arbiter per ROM grafica SD Gundam.
...
// Costo: 3× 256 entry × (32 data + 9 tag + 1 valid) = ~32 Kbit (~3 M10K).
reg [31:0] cache_data_bg [0:255];
reg [31:0] cache_data_mg [0:255];
reg [31:0] cache_data_fg [0:255];
reg [8:0]  cache_tag_bg  [0:255];
reg [8:0]  cache_tag_mg  [0:255];
reg [8:0]  cache_tag_fg  [0:255];
reg [255:0] cache_valid_bg;
reg [255:0] cache_valid_mg;
reg [255:0] cache_valid_fg;
...
always @(posedge clk) begin
	if (reset) begin
		cache_valid_bg <= 256'b0;
		cache_valid_mg <= 256'b0;
		cache_valid_fg <= 256'b0;
		r0_req_prev  <= 0;
		r1_req_prev  <= 0;
		r2_req_prev  <= 0;
		r3_req_prev  <= 0;
